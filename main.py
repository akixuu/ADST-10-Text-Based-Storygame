...
# text effects
class txt_effects:
	def dotdotdot(delay=0.05):
		for x in range(3):
			txt_effects.type_print('.', delay)
		print("\n")
	...
	def color_print(colorcode, text, typed=False, delay=0.05):
		if typed:
			txt_effects.type_print(colorcode + text + white, delay)
		else:
...
user["color"]=input("Welcome to "+ red + "AmongSus 2.0" + white + " - a luck based murder mystery. \n\nFirst, select your avatar color. We have red, yellow, green, blue, cyan and black. Take your pick: ")
while user["color"] not in characters.npcs: 
	user["color"]=input("Sorry, we don't have that color, or you may have a typo. Please try again: ")

print('\n')
txt_effects.dotdotdot(0.5)
# remove user color from main npc list
characters.npcs.remove(user["color"])

print("\nYou have chosen ", end='')
txt_effects.color_print(eval(user["color"]), user["color"])
print(".\n")

delay(1)

print("You are a astronaut on a mission to space, but recently, there has been suspicious activity going around in the spaceship. Many crew members theorize that there might be a  Your mission is to uncover ")

txt_effects.color_print(eval(user["color"]), assets.tinysus)
txt_effects.type_print("  Welcome aboard, \033[1mcrewmate.\033[0m")
